...
3. CamsAI_Invoice_Audit_Log_Output.csv — Live audit log output from the pipeline run, showing 126 unique invoices processed across 3 mock scenarios and 123 real invoice images sourced from the Voxel51 and mychen76 HuggingFace datasets. Each row captures the timestamp, vendor, total amount, compliance score, decision, anomaly flags, and ledger or alert ID.
